...
function BuildPersona () {
  ...
  const createPersona = async () => {  
    ...
    try {
      const response = await fetch('http://localhost:8000/create-persona', {
        method: 'POST',
        headers: {
      ...
      const newPersona = {
        ...
        avatar: avatar,
        name: personaData.profile.name,
        age: personaData.profile.age,        
        gender: gender, 
        diagnosis: medicalCondition, 
        occupation: personaData.profile.occupation,
        desc: descriptionText, 
        theme: personaData.theme,
        chats:[], 
        markedQuestions:[]
      };

      newPersona.systemPrompt = data.data.system_prompt; 
      newPersona.assistantMessage = data.data.assistant_message;
      ...
      window.alert("Failed to create persona, please try again.");
    } finally {
      setLoading(false); 
  }
  }
    ...
    <>
      {loading ? (
        <Box display="flex" alignItems="center" ml={15}>
          <CircularProgress />
          <span style={{ marginLeft: '10px', fontWeight: 'bold', fontSize: 18 }}>
            The persona is being created. Please wait...
          </span>
        </Box>
      ) : (
        value.personas.length !== 0 ? (
          ...
          >
         {
          ...
          <Box display="flex" flexDirection="column" justifyContent="center" alignItems="center" height="100vh" width="80%" sx={{ fontSize: '20px', color: '#666', margin: 'auto' }}>
        ...
        <Box display="flex" alignItems="center" justifyContent="center" py={3} px={10} gap={50}>
